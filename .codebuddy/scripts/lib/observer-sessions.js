...
function resolveProjectRoot(cwd = process.cwd()) {
  const envRoot = process.env.codebuddy_PROJECT_DIR;
...
function resolveSessionId(rawSessionId = process.env.codebuddy_SESSION_ID) {
  return sanitizeSessionId(rawSessionId || '') || '';
}

function getSessionLeaseFile(context, rawSessionId = process.env.codebuddy_SESSION_ID) {
  const sessionId = resolveSessionId(rawSessionId);
  if (!sessionId) return '';
  return path.join(getSessionLeaseDir(context), `${sessionId}.json`);
}

function writeSessionLease(context, rawSessionId = process.env.codebuddy_SESSION_ID, extra = {}) {
...
function removeSessionLease(context, rawSessionId = process.env.codebuddy_SESSION_ID) {
  const leaseFile = getSessionLeaseFile(context, rawSessionId);
