 ...
 *   2. Standard install location (~/.codebuddy/) — when scripts exist there
 *   3. Known plugin roots under ~/.codebuddy/plugins/ (current + legacy slugs)
 *   4. Plugin cache auto-detection — scans ~/.codebuddy/plugins/cache/{ecc,everything-claude-code}/
 *   5. Fallback to ~/.codebuddy/ (original behaviour)
 *
...
function resolveEccRoot(options = {}) {
  ...
  const claudeDir = path.join(homeDir, '.codebuddy');
  const probe = options.probe || path.join('scripts', 'lib', 'utils.js');

  // Standard install — files are copied directly into ~/.codebuddy/
  ...
  // ~/.codebuddy/plugins/cache/<plugin-name>/<org>/<version>/
  try {
