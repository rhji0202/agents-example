 ...
 * Cost Tracker Hook
 *
 * Appends lightweight session usage metrics to ~/.codebuddy/metrics/costs.jsonl.
 */
...
process.stdin.on('end', () => {
  try {
    ...
    const model = String(input.model || input._cursor?.model || process.env.codebuddy_MODEL || 'unknown');
    const sessionId = String(process.env.codebuddy_SESSION_ID || 'default');
