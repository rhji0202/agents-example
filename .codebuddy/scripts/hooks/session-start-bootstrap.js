 ...
 *   2. ~/.codebuddy (direct install)
 *   3. Several well-known plugin sub-paths under ~/.codebuddy/plugins/ (current + legacy)
 *   4. Versioned cache directories under ~/.codebuddy/plugins/cache/{ecc,everything-claude-code}/
 *   5. Falls back to ~/.codebuddy if nothing else matches
 *
 * @returns {string}
 */
function resolvePluginRoot() {
  const envRoot = process.env.codebuddy_PLUGIN_ROOT || '';
  if (hasRunnerRoot(envRoot)) {
    return path.resolve(envRoot.trim());
  }

  const home = require('os').homedir();
  const claudeDir = path.join(home, '.codebuddy');
  ...
  // Walk versioned cache: ~/.codebuddy/plugins/cache/{ecc,everything-claude-code}/<org>/<version>/
  try {
