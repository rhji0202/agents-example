...
```bash
mkdir -p .codebuddy/PRPs/reports
```

Write report to `.codebuddy/PRPs/reports/{plan-name}-report.md`:
...
```bash
mkdir -p .codebuddy/PRPs/plans/completed
mv "$ARGUMENTS" .codebuddy/PRPs/plans/completed/
```
...
### Artifacts
- Report: `.codebuddy/PRPs/reports/{name}-report.md`
- Archived Plan: `.codebuddy/PRPs/plans/completed/{name}.plan.md`
